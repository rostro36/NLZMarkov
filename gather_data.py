...
def gather(query,path):
    ...
    data=re.sub('.*<span id="totalCounter">','',data,1,re.DOTALL)               #determine how many results there are
    results=''
    for char in data:
        if char=='<':
            break
    ...
    number_of_pages=int((int(results)-1)/10+1)                                  #instead of math.ceil, determine how many sites of results there are
    for resultpage in range(1,number_of_pages+1):
        resultdata=http.request('GET',URL+'&page='+str(resultpage))             #download the result page
        resultdata=resultdata.data.decode("utf-8")
        datas=re.split('<a class="teaser__link " href="',resultdata)
        pageid=0
        for result in datas[1:]:                                                #go through the results given by the page of the results
            ...
            for text in sitedatas[1:]:                                          #there are possibly many sectons of the main text
